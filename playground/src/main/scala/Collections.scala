object Collections extends App {
    ...
    val spatialRecordsIterator = Iterator.tabulate(5)(i => SpatialRecord(i+scala.util.Random.nextInt(250), scala.util.Random.nextInt(2) match {
        case 0 => false
        case 1 => true
    }))

    val newList = spatialRecordsIterator.foldLeft(List.empty[SpatialRecord])((acc, sr) => {
        if(acc.isEmpty) {
            List(sr)
        } else {
            val last = acc.last
